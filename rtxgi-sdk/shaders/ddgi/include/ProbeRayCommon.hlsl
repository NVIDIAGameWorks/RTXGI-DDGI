...
void DDGIStoreProbeRayMiss(RWTexture2D<float4> RayData, uint2 coords, DDGIVolumeDescGPU volume, float3 radiance)
{
    if (volume.probeRayDataFormat == RTXGI_DDGI_FORMAT_PROBE_RAY_DATA_R32G32B32A32_FLOAT)
    {
        RayData[coords] = float4(radiance, 1e27f);
    }
    else if (volume.probeRayDataFormat == RTXGI_DDGI_FORMAT_PROBE_RAY_DATA_R32G32_FLOAT)
    {
        RayData[coords] = float4(asfloat(RTXGIFloat3ToUint(radiance)), 1e27f, 0.f, 0.f);
    }
}

void DDGIStoreProbeRayFrontfaceHit(RWTexture2D<float4> RayData, uint2 coords, DDGIVolumeDescGPU volume, float3 radiance, float hitT)
{
    if (volume.probeRayDataFormat == RTXGI_DDGI_FORMAT_PROBE_RAY_DATA_R32G32B32A32_FLOAT)
    {
...
void DDGIStoreProbeRayFrontfaceHit(RWTexture2D<float4> RayData, uint2 coords, DDGIVolumeDescGPU volume, float hitT)
{
    if (volume.probeRayDataFormat == RTXGI_DDGI_FORMAT_PROBE_RAY_DATA_R32G32B32A32_FLOAT)
    {
...
void DDGIStoreProbeRayBackfaceHit(RWTexture2D<float4> RayData, uint2 coords, DDGIVolumeDescGPU volume, float hitT)
{
...
float3 DDGILoadProbeRayRadiance(RWTexture2D<float4> RayData, uint2 coords, DDGIVolumeDescGPU volume)
{
    if (volume.probeRayDataFormat == RTXGI_DDGI_FORMAT_PROBE_RAY_DATA_R32G32B32A32_FLOAT)
    {
...
float DDGILoadProbeRayDistance(RWTexture2D<float4> RayData, uint2 coords, DDGIVolumeDescGPU volume)
{
    if (volume.probeRayDataFormat == RTXGI_DDGI_FORMAT_PROBE_RAY_DATA_R32G32B32A32_FLOAT)
    {
