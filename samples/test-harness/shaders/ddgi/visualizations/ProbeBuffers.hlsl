...
{
    #if SPIRV == 1
    uint volumeSelect = GetRootConstant(RasterRootConstants, volumeSelect);
    #endif
    ...
    // Irradiance
    float2 irradianceRect = numProbes.xy * numIrradianceProbeTexels * GetRootConstant(RasterRootConstants, VizIrradianceScale);
    ...
    // Filtered Distance
    float2 distanceRect = numProbes.xy * numDistanceProbeTexels * GetRootConstant(RasterRootConstants, VizDistanceScale);
        ...
        input.position.y <= (irradianceRect.y + distanceRect.y))
    {
        ...
        return float4((2.f * DDGIProbeDistanceSRV.SampleLevel(PointSampler, coords, 0).rg) / GetRootConstant(RasterRootConstants, VizDistanceDivisor), 0.f, 1.f);
    }

    // RT Radiance
    float2 radianceRect = float2(DDGIVolume.numRaysPerProbe, DDGIVolume.probeGridCounts.x * DDGIVolume.probeGridCounts.y * DDGIVolume.probeGridCounts.z) * GetRootConstant(RasterRootConstants, VizRadianceScale);
    ...
    {
        coords = float2(input.position.x, (input.position.y - irradianceRect.y - distanceRect.y)) / GetRootConstant(RasterRootConstants, VizRadianceScale);
    ...
    // Offsets
    float2 offsetRect = numProbes.xy * GetRootConstant(RasterRootConstants, VizOffsetScale);
        ...
        input.position.y <= (irradianceRect.y + distanceRect.y + radianceRect.y + offsetRect.y))
    {
        ...
        coords = float2(input.position.x, (input.position.y - irradianceRect.y - distanceRect.y - radianceRect.y)) / GetRootConstant(RasterRootConstants, VizOffsetScale);
    ...
    float2 stateRect = numProbes.xy * GetRootConstant(RasterRootConstants, VizStateScale);
#if RTXGI_DDGI_PROBE_RELOCATION
    ...
    {
        coords = float2(input.position.x, (input.position.y - irradianceRect.y - distanceRect.y - radianceRect.y - offsetRect.y - 1)) / GetRootConstant(RasterRootConstants, VizStateScale);
    ...
    {
        coords = float2(input.position.x, (input.position.y - irradianceRect.y - distanceRect.y - radianceRect.y - 1)) / GetRootConstant(RasterRootConstants, VizStateScale);
        return float4(float(DDGIProbeStates.Load(coords).r), 0.f, 0.f, 1.f);
    }
