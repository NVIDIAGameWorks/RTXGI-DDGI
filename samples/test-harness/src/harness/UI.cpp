    ...
    PostProcessOptions &postOptions)
{
    ...
    ImGui::Text("Camera");
    ImGui::Text("Position: (%.2f, %.2f, %.2f)", camera.origin.x, camera.origin.y, camera.origin.z);
    ...
    pos = (ImGui::GetWindowWidth() / 2.f) - ImGui::CalcTextSize("Ray Tracing").x / 2.f;
    ImGui::SetCursorPosX(pos);
    ...
    ImGui::DragInt("##rtNumBounces", &numBounces, 1, 0.f, 20.f, "Path Tracer Bounces: %.0f");
    ImGui::SameLine(); ShowHelpMarker("Adjust the number of bounces the path tracer uses.");
    ...
    pos = (ImGui::GetWindowWidth() / 2.f) - ImGui::CalcTextSize("RTXGI DDGIVolume").x / 2.f;
    ImGui::SetCursorPosX(pos);
    ImGui::Text("RTXGI DDGIVolume");
    ...
    ImGui::DragFloat("###hysteresis", &hysteresis, 0.01f, 0.f, 1.f, "Probe Hysteresis: %.2f");
    ...
    ImGui::DragFloat("###changeThreshold", &changeThreshold, 0.01f, 0.f, 1.f, "Probe Change Threshold: %.2f");
    ...
    ImGui::DragFloat("###brightnessThreshold", &brightnessThreshold, 0.0001f, 0.f, 1.f, "Probe Brightness Threshold: %.4f");
    ...
    ImGui::DragFloat("###normalBias", &normalBias, 0.01f, 0.f, maxDistance, "Normal Bias: %.2f");
    ...
    ImGui::Checkbox("Relocate Probes", &inputOptions.runProbeRelocation);
    ...
    if (lights.lightMask & 0x00000001)
    {
        ...
        pos = (ImGui::GetWindowWidth() / 2.f) - ImGui::CalcTextSize("Directional Light").x / 2.f;
        ImGui::SetCursorPosX(pos);
    ...
    if (lights.lightMask & 0x00000010)
    {
        ...
        pos = (ImGui::GetWindowWidth() / 2.f) - ImGui::CalcTextSize("Point Light").x / 2.f;
        ImGui::SetCursorPosX(pos);
    ...
    if (lights.lightMask & 0x00000100)
    {
        ...
        pos = (ImGui::GetWindowWidth() / 2.f) - ImGui::CalcTextSize("Spot Light").x / 2.f;
        ImGui::SetCursorPosX(pos);
    ...
    // AO
    {
        ...
        pos = (ImGui::GetWindowWidth() / 2.f) - ImGui::CalcTextSize("Ambient Occlusion").x / 2.f;
        ImGui::SetCursorPosX(pos);
...
void Initialize(D3D12Info &d3d, D3D12Resources &resources, HWND window)
{
    ...
    CPUHandle.ptr += (handleIncrement * 20);        // IMGui font SRV is 20th on the descriptor heap

    D3D12_GPU_DESCRIPTOR_HANDLE GPUHandle = resources.cbvSrvUavHeap->GetGPUDescriptorHandleForHeapStart();
    GPUHandle.ptr += (handleIncrement * 20);
    ...
    PostProcessOptions &postOptions)
{
    ...
    CreateDebugWindow(d3d, dxr, config, camera, lights, volume, input, inputOptions, rtOptions, vizOptions, postOptions);
}
