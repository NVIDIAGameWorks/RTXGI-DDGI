...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
    ...
    // Report memory leaks. These will appear in the debugger output.
#if defined _CRTDBG_MAP_ALLOC
    _CrtDumpMemoryLeaks();
#endif

    // If the run was not successful, notify the user to check the log file.
    if (result != EXIT_SUCCESS)
    {
        ::MessageBox(NULL, "Initialization error. See log.txt for details.", NULL, MB_OK);
    }
