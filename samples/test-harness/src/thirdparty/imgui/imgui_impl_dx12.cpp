...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
    ...
    if (fr->VertexBuffer == NULL || fr->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if(fr->VertexBuffer) fr->VertexBuffer->SetName(L"ImGui Vertex Buffer");
    }
    if (fr->IndexBuffer == NULL || fr->IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        if(fr->IndexBuffer) fr->IndexBuffer->SetName(L"ImGui Index Buffer");
    }
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        if(pTexture) pTexture->SetName(L"ImGui Fonts Texture");
        ...
        if(uploadBuffer) uploadBuffer->SetName(L"ImGui Fonts Upload Buffer");
        ...
        IM_ASSERT(SUCCEEDED(hr));
        if (cmdQueue) cmdQueue->SetName(L"ImGui Command Queue");

        ID3D12CommandAllocator* cmdAlloc = NULL;
        hr = g_pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&cmdAlloc));
        IM_ASSERT(SUCCEEDED(hr));
        if (cmdAlloc) cmdAlloc->SetName(L"ImGui Command Allocator");

        ID3D12GraphicsCommandList* cmdList = NULL;
        hr = g_pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAlloc, NULL, IID_PPV_ARGS(&cmdList));
        IM_ASSERT(SUCCEEDED(hr));
        if (cmdList) cmdList->SetName(L"ImGui Command List");
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        if(g_pRootSignature) g_pRootSignature->SetName(L"ImGui Root Signature");
    }
    ...
    if (g_pPipelineState) g_pPipelineState->SetName(L"ImGui PSO");
